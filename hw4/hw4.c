...
typedef struct USR
{
    int port,skfd,fd,status,bsuccess,sport;
	char ip[50],fname[50];
	char sip[30];
...
void setsocket(fd_set *rs,fd_set *ws)
{
    ...
    for(i=1; i<=5; i++)
    {
	    ...
	    printf("sport : %d\n",u[i].sport);
            ...
            if((u[i].he = gethostbyname(u[i].sip)) == NULL)
            {
            ...
            u[i].sin.sin_port = htons (u[i].sport);
			...
			request[0]=4;
			request[1]=1;
			request[2]=u[i].port/256;
			request[3]=u[i].port%256;
			int tmpip[4];
			sscanf(u[i].ip,"%d.%d.%d.%d",&tmpip[0],&tmpip[1],&tmpip[2],&tmpip[3]);
			request[4]=tmpip[0];
			request[5]=tmpip[1];
			request[6]=tmpip[2];
			request[7]=tmpip[3];	
			printf("dstip : %u.%u.%u.%u\n",request[4],request[5],request[6],request[7]);
			write(u[i].skfd,request,sizeof(request));
			usleep(3000);
			char replybuf[10];
			memset(replybuf,0,sizeof(replybuf));
			read(u[i].skfd,replybuf,sizeof(replybuf));
			if(replybuf[1]==91)
				continue;
...
int main()
{
    ...
    char teststr[]="h1=140.113.235.165&p1=8899&f1=t1.txt&sh1=140.113.235.165&sp1=9500&h2=&p2=&f2=&sh2=&sp2=&h3=&p3=&f3=&sh3=&sp3=&h4=&p4=&f4=&sh4=&sp4=&h5=&p5=&f5=&sh5=&sp5=";
//    parser(teststr);
